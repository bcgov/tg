...
export const updateSimulationForView = (
  ...
  switch (view) {
    ...
    case VIEW_OPTIONS.TREE_APPLICATIONS:
      simulation
        .force(
          "link",
          d3
          ...
          d3
            .forceX((d: Node) =>
              d.type === "application"
                ? (d.index! + 1) *
                  (width /
                    (nodes.filter((n) => n.type === "application").length + 1))
                : (d.index! + 1) *
                  (width /
                    (nodes.filter((n) => n.type === "technology").length + 1))
            )
            .strength(1)
        )
        .force(
          ...
          d3
            .forceY((d: Node) =>
              d.type === "application" ? height / 4 : (height * 3) / 4
            )
            .strength(1)
        );
      break;

    case VIEW_OPTIONS.TREE_TECHNOLOGIES:
      simulation
        .force(
          "link",
          d3
        ...
        .force(
          ...
          d3
            .forceY((d: Node) =>
              d.type === "technology" ? height / 4 : (height * 3) / 4
            )
            .strength(1)
        );
      break;
  ...
  if (searchQuery) {
    ...
    for (let i = 0; i < degree; i++) {
      ...
      links.forEach((link) => {
        ...
        if (
          nodesWithinDegrees.has(sourceId) ||
          nodesWithinDegrees.has(targetId)
        ) {
    ...
    simulation
      ...
      .force(
        ...
        d3
          .forceManyBody<Node>()
          .strength((d) => (matchedNodeIds.has(d.id) ? -30 : -100))
      )
      .force(
        "collide",
        d3
          .forceCollide<Node>()
          .radius((d) => (matchedNodeIds.has(d.id) ? 20 : 30))
      )
      .force(
        "x",
        d3
          .forceX<Node>((d: Node) =>
            matchedNodeIds.has(d.id) ? width / 2 : d.x ?? width / 2
          )
          .strength(0.1)
      )
      .force(
        ...
        d3
          .forceY<Node>((d: Node) =>
            matchedNodeIds.has(d.id) ? height / 2 : d.y ?? height / 2
          )
          .strength(0.1)
      );
    ...
    g
      .selectAll<SVGCircleElement, Node>("rect")
      .style("opacity", (d) => (nodesWithinDegrees.has(d.id) ? 1 : 0.2))
      .style("stroke", (d) => (matchedNodeIds.has(d.id) ? "purple" : null))
      .style("stroke-width", (d) => (matchedNodeIds.has(d.id) ? "3px" : null));
    ...
    g
      ...
      .style("opacity", (d) =>
        nodesWithinDegrees.has((d.source as Node).id) &&
        nodesWithinDegrees.has((d.target as Node).id)
          ? 1
          : 0.2
      );
  } else {
